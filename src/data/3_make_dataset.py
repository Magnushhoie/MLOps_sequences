...
def stack_res_tensors(tensors):
    ...
    # Initialize empty padded vector,
    # with 0-padding for sequences with less than max length residues
...
def get_PCA_fit_train(pca_dim=60):
    ...
    def PCA_fit(stacked_tensor, pca_dim=60):
        ...
        positions = stacked_tensor.view(
            stacked_tensor.shape[0] * stacked_tensor.shape[1], embedding_dim
        )
    ...
    tensor_files = glob.glob(interimDir + "train_pos/*.pt") + glob.glob(
        interimDir + "train_neg/*.pt"
    )
...
def PCA_transformed_save_embed_files(
    pca, embed_dim, max_len, dirType_list, fasta_list, interimDir, procDir
):
...
def prepare_dataset(ds_pos_folder, ds_neg_folder, dataDir):
    ...
    """
    X_tensor_pos, y_tensor_pos, df_pos = load_tensor_folder(
        ds_pos_folder, label_int=1, dataDir=dataDir
    )
    X_tensor_neg, y_tensor_neg, df_neg = load_tensor_folder(
        ds_neg_folder, label_int=0, dataDir=dataDir
    )
...
def main(interim_filepath, output_filepath):
    ...
    PCA_transformed_save_embed_files(
        pca_train, embed_dim, max_len, dirType_list, fasta_list, interimDir, procDir
    )

    # Prepare dataset
    # To-do: Could work directly with data from above function
    logger.info("Preparing train, validation and test TensorDataset")
    X_trainval, y_trainval, df_trainval = prepare_dataset(
        ds_pos_folder="train_pos", ds_neg_folder="train_neg", dataDir=procDir
    )
    X_test, y_test, df_test = prepare_dataset(
        ds_pos_folder="test_pos", ds_neg_folder="test_neg", dataDir=procDir
    )

    logger.info(f"Splitting of {val_frac} of training dataset into validation")
    X_train, y_train, df_train, X_valid, y_valid, df_valid = split_train_val(
        X_trainval, y_trainval, df_trainval, val_frac
    )
    ...
    # Save
    log.info(
        f"Saving trainset.pt ({trainset.tensors[0].shape}), trainset.csv to {procDir}"
    )
    df_train.to_csv(procDir + "trainset.csv", index=False)
    torch.save(trainset, procDir + "trainset.pt")

    log.info(
        f"Saving validset.pt ({validset.tensors[0].shape}), validset.csv to {procDir}"
    )
    df_valid.to_csv(procDir + "validset.csv", index=False)
    torch.save(validset, procDir + "validset.pt")

    log.info(
        f"Saving testset.pt ({testset.tensors[0].shape}), testset.csv to {procDir}"
    )
