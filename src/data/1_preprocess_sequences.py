...
def de_duplicate_FASTA_files(fastaList, outName, interimDir="data/interim/", v=1):
    """
    Reads and checks list of FASTA files,
    and saves individual ID+sequence FASTA to fastaOutdir in interimDir
    """
...
def main(input_filepath, interim_filepath):
    ...
    # Print statistics
    print(
        f"""\nProcessed {count_entries} sequences from {len(fasta_list_list)} FASTA
        files in {input_filepath} """
    )
